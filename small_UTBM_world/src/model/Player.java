...
public class Player {
	...
	public void declin() {
		// au cas où l'utilisateur ait déjà un peuple en déclin, on vide ces
		// salles
		...
		while (!declinedRooms.isEmpty()) {
			declinedRooms.remove(0);
		}
		int temp = nbRooms - 1;

		for (int i = 0; i <= temp; i++) {
			declinedRooms.add(rooms.remove(temp - i));
			declinedRooms.get(i).setOwner(
					NullPlayerSingleton.getInstance().getNullPlayer());
		...
		GlobalPanel.getInstance().showCombPanDecline(this);
	}
		...
		if (canConquer(r)) {
			if (r.getNbUnits() == 0) {
				...
				int nbNeeded = 2 + r.getNbUnits()
						...
						- pouv.bonusattack(r)
						- dep.attack(nbPawnsInHand, r.getRoomType());// rajouter
																		// le
																		// calcul
																		// du
																		// bonus
	...
	private boolean canConquer(Room r) {
		...
		while (i < nbRooms && rooms.get(i) != r) {
			System.out.println(rooms.get(i).getPosX() + " "
					+ rooms.get(i).getPosY());
